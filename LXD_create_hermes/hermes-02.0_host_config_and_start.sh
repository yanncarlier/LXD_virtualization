...
echo "============================================="
echo " Hermes Agent LXD VM 02.0_host_config_and_start.sh "
echo "============================================="

# ── 1. Confirm VM exists and is stopped ───────────────────────────────────────
STATUS=$(lxc list "${VM_NAME}" --format csv --columns s 2>/dev/null | head -1)
if [[ -z "${STATUS}" ]]; then
  echo "[!] VM '${VM_NAME}' not found. Run hermes-01_host_create_vm.sh first."
  exit 1
fi
...
# ── 4. Push in-VM setup scripts ────────────────────────────────────────────────
echo "[*] Pushing in-VM setup scripts..."
lxc file push hermes-03.0_vm_setup.sh "${VM_NAME}/${VM_USER}/hermes-03.0_vm_setup.sh"
lxc file push hermes-03.1_vm_setup.sh "${VM_NAME}/${VM_USER}/hermes-03.1_vm_setup.sh"
lxc file push hermes-04.0_vm_setup.sh "${VM_NAME}/${VM_USER}/hermes-04.0_vm_setup.sh"

lxc exec "${VM_NAME}" -- chmod +x /${VM_USER}/hermes-03.0_vm_setup.sh
lxc exec "${VM_NAME}" -- chmod +x /${VM_USER}/hermes-03.1_vm_setup.sh
lxc exec "${VM_NAME}" -- chmod +x /${VM_USER}/hermes-04.0_vm_setup.sh
